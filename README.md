...
<head>
  ...
  <base href="https://cdn.jsdelivr.net/gh/bubbls/UGS-Assets@main/fall-guys/">
...
<body>
  ...
  <script type="module">
    ...
    'use strict';
    {
      ...
      function AddScript(url, type) {
        ...
        elem.async = false;
        if (type === "module") elem.type = "module";
        if (url.isStringSrc) {
      ...
      window.RuntimeInterface = class RuntimeInterface {
        constructor(opts) {
          ...
          this._scriptsType = opts.scriptsType;
          ...
          if (this._useWorker &&
            (typeof OffscreenCanvas === "undefined" || !navigator["userActivation"] || this._scriptsType === "module" && !SupportsWorkerTypeModule())) this._useWorker = false;
          ...
          if ((this._exportType === "html5" || this._exportType === "playable-ad") && location.protocol.substr(0,
              4) === "file") alert("Exported games won't work until you upload them. (When running on the file: protocol, browsers block many features from working for security reasons.)");
        ...
        async _Init(opts) {
          ...
          if (this._useWorker) await this._InitWorker(opts,
        ...
        async _InitDOM(opts, port2) {
          ...
          engineScripts = await Promise.all(engineScripts.map(url => this._MaybeGetCordovaScriptURL(url, this._scriptsType)));
          await Promise.all(engineScripts.map(url => AddScript(url, this._scriptsType)));
          const scriptsStatus = self["C3_ProjectScriptsStatus"];
          if (this._scriptsType === "module") {
            ...
            for (let [originalUrl, loadUrl] of allProjectScripts) {
              ...
              if (originalUrl === mainProjectScript) try {
                loadUrl = await this._MaybeGetCordovaScriptURL(loadUrl, this._scriptsType);
                await AddScript(loadUrl, this._scriptsType);
                if (this._exportType === "preview" && !scriptsStatus[originalUrl]) this._ReportProjectMainScriptError(originalUrl,
          ...
          } else if (opts.projectScripts && opts.projectScripts.length > 0) try {
            await Promise.all(opts.projectScripts.map(e => AddScript(e[1], this._scriptsType)));
            if (Object.values(scriptsStatus).some(f => !f)) {
              self.setTimeout(() =>
                this._ReportProjectScriptError(scriptsStatus), 100);
              return
            }
          } catch (err) {
        ...
        async _MaybeGetCordovaScriptURL(url, type) {
          if (this._exportType === "cordova" && type === "module" && (url.startsWith("file:") || location.protocol === "file:" && this.IsRelativeURL(url))) {
            let filename = url;
    ...
    'use strict';
    {
      const DISPATCH_WORKER_SCRIPT_NAME = "scripts/dispatchworker.js";
      const JOB_WORKER_SCRIPT_NAME = "scripts/jobworker.js";
    ...
    'use strict';
    {
      if (window["C3_IsSupported"]) {
        ...
        window["c3_runtimeInterface"] = new self.RuntimeInterface({
          ...
          engineScripts: ["https://cdn.jsdelivr.net/gh/bubbls/UGS-Assets@main/fall-guys/scripts/c3runtime.js"],
          projectScripts: [],
          mainProjectScript: "",
          scriptsType: "module",
  ...
  <body onkeydown="checkKey (event)">
    <script defer src="https://static.cloudflareinsights.com/beacon.min.js/vcd15cbe7772f49c399c6a5babf22c1241717689176015" integrity="sha512-ZpsOmlRQV6y907TI0dKBHq9Md29nnaEIPlkf84rnaERnq6zvWvPUqr2ft8M1aS28oN72PdrCzSjY4U6VaAw1EQ==" data-cf-beacon='{"version":"2024.11.0","token":"a1aa08033bf84b91b93e262e3aac75e3","r":1,"server_timing":{"name":{"cfCacheStatus":true,"cfEdge":true,"cfExtPri":true,"cfL4":true,"cfOrigin":true,"cfSpeedBrain":true},"location_startswith":null}}' crossorigin="anonymous"></script>
  </body>
